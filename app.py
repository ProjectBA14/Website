    ...
    if request.method == 'POST':
        ...
        if ticket_id:
            try:
                ...
                if "unassign" in it_executives:
                    # Remove all specified IT executives from the ticket
                    for exec_email in it_executives:
                        ticket_ref.update({
                            'assigned_to': firestore.ArrayRemove([exec_email])
                        })
                    # If no IT executives remain, change the status to "Open"
                    ...
                    # Add specified IT executives to the ticket
                    ticket_ref.update({
        ...
        # Update the scheduled event if the assignment is for an event
            ...
            try:
                ...
                if "unassign" in it_executives:
                    # Remove all specified IT executives from the event
                    for exec_email in it_executives:
                        event_ref.update({
                            'assigned_to': firestore.ArrayRemove([exec_email])
                        })
                    ...
                    # Add specified IT executives to the event
                    event_ref.update({
                        'assigned_to': firestore.ArrayUnion(it_executives)
                    })
                    flash("Scheduled event successfully assigned to selected IT executive(s).")
            except Exception as e:
                flash(f"Error updating scheduled event: {str(e)}")
